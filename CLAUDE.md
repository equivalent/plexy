...
- **Interactive UI: use [phlex-reactive](https://phlex-reactive.zoolutions.llc) by default** (Livewire-style components), not hand-rolled Stimulus/Turbo Streams. Reference example: `app/components/product_card.rb`. Prefer `reactive_record` over `reactive_state` when data lives in a model. Run `bin/rails phlex_reactive:doctor` after changes.
